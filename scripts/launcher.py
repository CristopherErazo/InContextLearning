...
def build_controller(cfg, log_metrics=None, log_to_terminal=True) -> TrainerController:
    ...
    def eval_art_fun():
        with torch.no_grad():
            input = test_batch["sequence"][:, :-1].to(device)
            mask = test_batch["mask"].to(device)
            output = model.full_output(input, mask=mask)
